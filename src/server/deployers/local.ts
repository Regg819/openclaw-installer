...
| --- | --- | --- | --- | --- |`;
    ...
    const initScript = [
      ...
      // Copy agent source files, skipping runtime state files owned by the agent (MEMORY.md, BOOTSTRAP.md)
      `for d in /tmp/agent-source/workspace-*; do if [ -d "$d" ]; then base="$(basename "$d")"; ${mainWorkspaceShellCondition(workspaceDir, sourceBundle)}; mkdir -p "$dest"; for f in "$d"/*; do fname="$(basename "$f")"; case "$fname" in MEMORY.md|BOOTSTRAP.md) continue ;; esac; cp -r "$f" "$dest/$fname" 2>/dev/null || true; done; fi; done`,
  ...
  async start(result: DeployResult, log: LogCallback): Promise<DeployResult> {
    ...
    if (hasWorkspaceDirs) {
      ...
      const copyScript = [
        ...
        `      fname="$(basename "$f")"`,
        `      case "$fname" in`,
        `        MEMORY.md|BOOTSTRAP.md) continue ;;`,
        `      esac`,
        `      cp -r "$f" "$dest/$fname" 2>/dev/null || true`,
        `    done`,
        `  fi`,
        `done`,
  ...
  async redeploy(result: DeployResult, log: LogCallback): Promise<void> {
    ...
    const copyScript = [
      ...
      `        MEMORY.md|BOOTSTRAP.md) continue ;;`,
      `      esac`,
      `      cp -vr "$f" "$dest/$fname" 2>/dev/null || true`,
      `    done`,
      `  fi`,
      `done`,
